...
- 模型推理
- 模型部署
description: Page Attention是优化KV-cache内存管理的方法，可解决预留浪费、内部及外部内存碎片化问题，通过将KV-cache划分为固定大小Block，利用Block
  Table维护逻辑与物理映射，有效管理非连续内存；同时处理Softmax计算中的数值溢出与全局信息问题。vllm框架支持离线与在线调用，初始化过程包括模型加载、KV
  Cache显存预分配（计算可用内存、num_blocks及总token数）并加载到GPU，提升内存使用效率。
---
## PageAttention原理分析
...
![](https://s2.loli.net/2025/12/27/NYdBtxTf36EuqwF.png)
...
### vllm初始化过程
按照PPT[^2]中对于模型加载的描述：
![](https://s2.loli.net/2025/12/27/18KgiQjdlGkHfMR.png)
...
![](https://s2.loli.net/2025/12/28/vxUXTZ1c3yRjDQr.png)
...
![](https://s2.loli.net/2025/12/28/2tjvz4prRCu6bZg.png)
...
在函数`self._allocate_kv_cache_tensors`中很容易理解直接初始化一个全部为0的张量，而后再去通过函数`_reshape_kv_cache_tensors`将张量的形状改为`[num_blocks, block_size, num_kv_heads, head_size]`
## 总结
