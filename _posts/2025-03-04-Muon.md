...
description: AdamW优化器作为主流优化器，是Adam的变种，通过一阶矩（梯度加权平均）和二阶矩（梯度平方加权平均）估计、偏置校正，以及在更新规则中加入权重衰减改进L2正则化。而新优化器Muon（MomentUm
  Orthogonalized by Newton-Schulz）则通过获取SGD-momentum生成的更新并应用Newton-Schulz迭代后处理步骤优化二维神经网络参数，属自适应学习率优化器，类似Adam。Muon对参数形状有要求，不可直接用于embedding层及最后全连接层（需用其他优化器），4D卷积滤波器建议展平最后三维。其优势在于显存占用低（仅存储动量，AdamW需存一阶、二阶矩），但存在额外计算延迟。Kimi论文中对Muon的改进包括增加Weight
  Decay以解决低精度训练时的收敛速度问题，以及通过学习率调节增加RMS控制使更新步RMS与Adam一致。Muon在Transformer模型上表现优异，使用时需注意参数划分，是值得关注的高效优化器。
---
...
<img src="https://s2.loli.net/2025/06/21/lXyH48toiDEIksc.webp" alt="image" width="682" height="461" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/YIjPow4xSMqgfba.webp" alt="image" width="1536" height="545" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/iFNa89VpqCKkTzZ.webp" alt="image" width="1181" height="588" loading="lazy" decoding="async" />
