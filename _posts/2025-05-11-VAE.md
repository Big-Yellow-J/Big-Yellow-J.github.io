...
- 生成模型
- VAE
description: 本文详细介绍变分自编码器（VAE）的基本原理、数学框架及代码实战，并对比自编码器（AE）与VAE的差异。AE作为无监督学习模型，通过编码-解码学习数据特征表示，但因潜在空间缺乏明确结构不适合生成任务；而VAE结合概率模型与深度学习，将输入映射为概率分布（如高斯分布），通过变分推理使潜在空间具备概率特性，更适合图像生成。VAE核心目标是学习数据概率分布p(x)，通过编码器（神经网络）用变分分布近似后验分布，解码器基于采样的潜在变量重构输入，实现新样本生成。其损失函数包含重构项（衡量生成数据与真实数据差异）和KL散度（优化近似分布与先验分布的相似度）。此外，文章还介绍VQ-VAE通过离散码本（codebook）解决VAE生成模糊问题，将编码器输出映射到码本向量，提升局部结构捕捉能力。代码实战部分以MNIST数据集为例，构建包含encoder、decoder及潜在空间处理的模型，重点实现潜在变量的均值（μ）、标准差（σ）拟合及随机变量采样，通过梯度下降优化损失函数。本文涵盖贝叶斯统计、变分推理、生成模型等关键技术，为VAE理论学习与实践应用提供全面指导。
---
...
<img src="https://s2.loli.net/2025/06/21/WHTJDpLjXBcyuhb.webp" alt="image" width="903" height="265" loading="lazy" decoding="async" />
...
大致总结一下上述内容，**VAE的主要的任务**：从最开始的数据集里面去获取数据的潜在分布进行学习，如果我模型学会了各类数据的分布，那么就可以通过这些分布去进一步生成新的数据。
...
$$
> 两项计算看似复杂，在实际应用过程中**模型分为两部分**encoder、decoder首先通过encoder去获取图像编码之后通过**特定网络结构**（比如说一层线性层）去获取编码特征的分布系数 $\mu$ 和 $\sigma$，而在得到分布系数之后直接用分布系数交给decoder是不行的（以为得到只是分布系数，直接交给decoder不可导）因此需要一次 “模拟生成”（用一个随机噪声+数据分布比如说直接 $\mu+ \sigma * ϵ$）在得到 “模拟生成之后”直接让decoder进行解码即可，回到**loss计算过程**：1、第一项直接去计算模型持重构图像和实际图像之间BCEloss；2、第二项直接去计算 `KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())`

上述等式中，第一项表示的是给定从编码器得到的潜变量 $z$，解码器“生成”原始数据 $x$ 的能力有多强，第二项表示的是编码器学到的 latent 分布和我们预先设定的先验分布 p（通常是标准正态 N(0,I)）之间的差异。 在深度学习中自然就会直接用 **梯度下降**的方法去优化参数，下面推荐借鉴：[https://arxiv.org/pdf/1906.02691](https://arxiv.org/pdf/1906.02691) 中的描述，**参数求解之前引入最开始定义的几个变量**：
$$
...
<img src="https://s2.loli.net/2025/06/21/F5rtGRuhl9obVSX.webp" alt="image" width="949" height="478" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/dro6EVse1yBXh3j.webp" alt="image" width="1280" height="382" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/R283pxYnkLqTfsZ.webp" alt="image" width="770" height="487" loading="lazy" decoding="async" />
...
所有的代码：1、[VAE](https://github.com/shangxiaaabb/ProjectCode/blob/main/code/Python/ModelTraining/VAE/VAE.py)；2、[PixelCNN](https://github.com/shangxiaaabb/ProjectCode/blob/main/code/Python/ModelTraining/VAE/PixcelCNN.py)
...
| 固定输入 | 重构图像 | 随机生成 |
|:--------:|:---------:|:--------:|
| <img src="https://s2.loli.net/2025/05/14/ABJDCTjW46Odx3g.gif" alt="image" width="152" height="152" loading="lazy" decoding="async" /> |<img src="https://s2.loli.net/2025/05/14/fQmMFOXRNovIubA.gif" alt="image" width="152" height="302" loading="lazy" decoding="async" /> | <img src="https://s2.loli.net/2025/05/14/28Tln5qcap6HvPf.gif" alt="image" width="152" height="152" loading="lazy" decoding="async" /> |
...
|:------:|:----------------:|:-----------------------:|:--------------------:|
|<img src="https://s2.loli.net/2025/05/14/38zIQHaL9Vukret.gif" alt="image" width="172" height="342" loading="lazy" decoding="async" />| <img src="https://s2.loli.net/2025/05/15/DHvAXfZIRimwOen.gif" alt="image" width="172" height="172" loading="lazy" decoding="async" /> | <img src="https://s2.loli.net/2025/05/15/mQqgLNF83uzoZiJ.gif" alt="image" width="172" height="172" loading="lazy" decoding="async" /> | <img src="https://s2.loli.net/2025/05/15/gDcBlhsyS1or8Iu.gif" alt="image" width="172" height="172" loading="lazy" decoding="async" />|
