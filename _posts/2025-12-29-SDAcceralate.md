...
# convert 处理得到模型为
(0): Linear(in_features=32, out_features=128, bias=True, weight=Float8Tensor(self.act_quant_kwargs=None, self.block_size=[1, 32], self.mm_config=None, self.scale.shape=torch.Size([128, 1]), self.kernel_preference=<KernelPreference.AUTO: 'auto'>))
```

从上面模型变化过程可以看到，在 prepare 阶段，会将模型进行fake包裹，而后通过训练得到量化的模型权重，再通过 convert 处理，将模型中的权重和激活值都进行了量化（权重变为float8激活值变为int8） -->
...
vllm不仅仅对llm可以起作用，在diffuseion model中也可以起作用，可以直接使用 vllm-onmi[^6]进行扩散模型加速推理其内部逻辑为：
