...
```
然后将 `from_pretrained` 里面的 `quantization_config` 修改即可，在 torchao 中支持的量化方式有：[torchao.quantization](https://docs.pytorch.org/ao/stable/api_reference/api_ref_quantization.html#torchao-quantization)，而对于 **TorchAO 的量化核心**基于仿射量化和 分组/通道级粒度，底层依赖自定义内核（CUDA/XPU/CPU）与 inductor 编译器融合。
