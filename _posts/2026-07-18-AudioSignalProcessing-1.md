...
## VAD模型简要概述
VAD 的核心任务是 **判断一段音频中哪些时间段有人说话哪些时间段没有人说话**（也就是他就是一个音频是不是有静音的检测器），也就是区分语音和非语音（或静音）部分。想象一下，在一个有背景噪音的环境中，VAD 就像一个智能“守门人”，它能准确识别什么时候有人在说话，什么时候是纯粹的环境噪音或沉默。
### VAD模型工作原理
...
### FSMN-VAD — 前馈序列记忆网络
> [https://modelscope.cn/models/iic/speech_fsmn_vad_zh-cn-16k-common-pytorch/summary](https://modelscope.cn/models/iic/speech_fsmn_vad_zh-cn-16k-common-pytorch/summary)

#### 模型结构
阿里达摩院在 FunASR 中开源的 VAD 模块 [^3]，基于前馈序列记忆网络（FSMN），专为中英文语音场景优化，仅 0.4M 参数。其具体模型结构：
![](https://modelscope.cn/models/iic/speech_fsmn_vad_zh-cn-16k-common-pytorch/resolve/master/fig/struct.png)

**特征提取**[^3]：80 维 Fbank（梅尔滤波器组能量），帧长 25ms，帧移 10ms。
**模型结构**：在传统的序列建模模型RNN/LSTM等在第 t 帧的结果依赖第 t−1 帧的隐藏状态，所以无法并行化，训练和推理都很慢，FSMN 的核心洞察是：序列建模不一定需要递归。如果你能让每一帧直接"看到"前后若干帧的信息，就能用纯前馈网络（可并行）达到类似的上下文建模效果，因此FSMN 的核心思想是在标准前馈网络（FFN）的隐藏层中插入 **可学习的记忆模块 (Memory Block)**：

$$\tilde{h}_t = h_t + \sum_{i=0}^{N} a_i \odot h_{t-i} + \sum_{j=1}^{M} c_j \odot h_{t+j}$$

其中 $h_t$ 是当前帧的隐藏向量，$a_i, c_j$ 是前向/后向记忆权重。相比 LSTM/GRU，FSMN 无需序列化递归计算，所有帧可并行前向传播，训练和推理都更快。对于上述个公式在具体的模型结构设计中对于输入特征维度 $(B,T,D)$ 直接可以通过循环去对时间维度 $T$ 进行读取然后进行编码即可，实际上更加高效的计算直接通过卷积进行实现：
```python
class MemoryBlock(nn.Module):
  """FSMN 核心：可学习的固定窗口上下文加权模块。
  
  公式: h̃_t = h_t + Σ a_i ⊙ h_{t-i} + Σ c_j ⊙ h_{t+j}
  
  关键设计：a_i, c_j 是 d 维向量（不是矩阵），参数量仅 O(d × (N+M))，
  远小于自注意力的 O(d²)。
  """
  
  def __init__(self, dim: int, forward_order: int, backward_order: int):
      """
      Args:
          dim: 隐藏层维度 d
          forward_order: 前向阶数 N（看过去多少帧）
          backward_order: 后向阶数 M（看未来多少帧）
      """
      super().__init__()
      self.N = forward_order   # 前向记忆阶数
      self.M = backward_order  # 后向记忆阶数
      
      # 可学习的记忆权重向量，形状 (N+1, dim) 和 (M, dim)
      # +1 是因为 i=0 也包含在内（当前帧自身的 weight）
      self.forward_weights = nn.Parameter(torch.randn(forward_order + 1, dim) * 0.02)
      self.backward_weights = nn.Parameter(torch.randn(backward_order, dim) * 0.02)
  
  def forward(self, x: torch.Tensor) -> torch.Tensor:
      """
      Args:
          x: (B, T, dim) — 批量 × 时间帧 × 隐藏维度
      Returns:
          (B, T, dim) — 融合上下文后的增强表示
      """
      B, T, d = x.shape
      ...
      # ---------- 逐帧计算（可优化为 conv1d，这里保持公式的直观性）----------
      for t in range(T):
          # --- 前向部分：Σ a_i ⊙ h_{t-i}，i = 0..N ---
          forward_sum = torch.zeros(B, d, device=x.device)
          for i in range(self.N + 1):
              idx = t - i
              if idx >= 0:
                  forward_sum = forward_sum + self.forward_weights[i] * x[:, idx, :]
          
          # --- 后向部分：Σ c_j ⊙ h_{t+j}，j = 1..M ---
          backward_sum = torch.zeros(B, d, device=x.device)
          for j in range(1, self.M + 1):
              idx = t + j
              if idx < T:
                  backward_sum = backward_sum + self.backward_weights[j - 1] * x[:, idx, :]
          
          out[:, t, :] = x[:, t, :] + forward_sum + backward_sum
      
      return out


class FastMemoryBlock(nn.Module):
  """MemoryBlock 的高效卷积实现版本。
  
  将逐元素加权求和等价为 depthwise 1D 卷积：
  - 前向部分等价于 causal conv1d (groups=dim)
  - 后向部分等价于 anti-causal conv1d (groups=dim)
  """
  
  def __init__(self, dim: int, forward_order: int, backward_order: int):
      super().__init__()
      self.N = forward_order
      self.M = backward_order
      
      # 前向权重：shape (dim, 1, N+1)，作为 depthwise conv 的 kernel
      # 卷积核排列：[w_0, w_1, ..., w_N]（从左到右 = 从当前到过去）
      fwd_kernel = torch.randn(dim, 1, forward_order + 1) * 0.02
      self.register_buffer('fwd_kernel', fwd_kernel)
      
      # 后向权重：shape (dim, 1, M)，右侧填充 0 作为 anti-causal
      bwd_kernel = torch.randn(dim, 1, backward_order) * 0.02
      self.register_buffer('bwd_kernel', bwd_kernel)
      
      # 用 Parameter 方式（支持训练）：
      self.fwd_weight = nn.Parameter(torch.randn(dim, 1, forward_order + 1) * 0.02)
      self.bwd_weight = nn.Parameter(torch.randn(dim, 1, backward_order) * 0.02)
  
  def forward(self, x: torch.Tensor) -> torch.Tensor:
      """
      x: (B, T, dim) → permute → (B, dim, T) 作为 depthwise conv 的输入
      """
      B, T, d = x.shape
      x_t = x.permute(0, 2, 1)  # (B, dim, T)
      
      # 前向 causal conv：padding 在左侧，只看过去
      fwd_out = F.conv1d(
          x_t, self.fwd_weight, groups=d,
          padding=self.N  # 左侧 pad N 个零（保证 causal）
      )[:, :, :T]  # 裁掉右侧多余的
      
      # 后向 anti-causal conv：padding 在右侧，只看未来
      bwd_out = F.conv1d(
          torch.flip(x_t, [2]),  # 时间反转
          self.bwd_weight, groups=d,
          padding=self.M
      )[:, :, :T]
      bwd_out = torch.flip(bwd_out, [2])  # 再反转回来
      
      out = x_t + fwd_out + bwd_out
      return out.permute(0, 2, 1)  # (B, T, dim)
```

不过在具体的“时序”数据中因为未来时间信息是未知的所以**训练/推理过程中比较有效的策略**：1、**离线推理过程**：训练过程中双向+推理使用双向，不过训练过程中直接使用 **Dropout** 随机不让 *未来* 信息融入；2、**实时推理**：此时需要将模型退化为因果FSMN（$tilde{h}_t = h_t + \sum_{i=0}^{N} a_i \odot h_{t-i}$），而为了保证实时推理的有效一般策略是使用 *状态累计策略*，通过固定采集音频进行输入而后对采集的音频中进行 “静音片段检测”当片段超过累计则进行放回，具体流程如下：
![](https://files.seeusercontent.com/2026/07/22/0Fdc/20260722233630743.png)
...
### FireRedVAD
#### 模型结构
小红书开源VAD模型[^5]模型参数上只有0.6M基于DFSMN同时支持流式和非流式的使用，[FSMN模型结构介绍](#fsmn-vad--前馈序列记忆网络)
#### 模型使用
...
**音频数据结构**一般对于输入的音频读取过程中，假设直接通过[torchaudio](https://docs.pytorch.org/audio/stable/generated/torchaudio.load.html#torchaudio.load)去读取 `waveform, sample_rate = torchaudio.load("test.wav")`对于两部分返回内容分别表示：1、`sample_rate`：采样率；2、`waveform`：`[channel, time]`表示通道数量以及音频采样点数量。比如说16Hz，3秒，单声道返回结果就是 `torch.Size([1, 48000])`。一般而言模型输入数据主要是两种：1、直接输入waveform数据；2、直接输入输入Mel/Fbank特征（会将音频的 waveform 数据进行转换）：对于输入为 `[1, 48000]`会直接转换为 `[T,F]`比如说 `[300, 80]`那么这样一来音频就变成 `[bs, 300, 80]`。
> 如果音频长度不一致一般就直接用padding策略即可

**标签构建过程**对于输入音频只需要标记出来中间那些位置有音频，比如说10s的音频设置10ms（每一秒100帧），那么0.80s ~ 3.50s对对应帧 80 到帧 349，全部标1依次类推这样一来**标签就变成了**：`[0,0,...,1,...1,0...0]`，但是对于训练数据如何去对音频截取窗口就需要考虑：因为很可能一段音频里面大部分都是静音的，这样就会导致模型效果不佳。一般的处理策略很多比如说有些ASR模型会返回每个字所对应的时间戳直接去根据这个时间戳截取即可。

* **损失函数设计**

**损失函数构建**可以直接使用二元交叉熵损失函数处理即可

* **推理过程**

对于输入音频进行读取获取1维的数组（如果双通道直接取平均即可）假设得到数据维度为 `[1, 3s x 16000hz]`，而后将1维的波形特征转化为2维矩阵特征假设得到维度信息为：`[1, 300帧, 80]`而后直接将这个音频输入到模型这哦你进行推理输出每一帧的概率 `[1,300,1]`。但是输出结果中可能出现在 t帧位置判断为静音（可能是错误判断），但是不能根据这个又把音频切分成两段，因此需要一个后处理过程最简单的就是直接静音帧累计策略（静音帧数累计>30就认为应该切分了）
> 除此之外有些时候还会使用 `speech_pad` 策略就是将预测的时间范围向前向后补充部分帧避免截断为题出现

**除此之外**还需要考虑超长音频吹过程（不可能直接全部输入到模型中，显存处理不下）具体过程如下（假设模型结构为FSMN）：
![](https://files.seeusercontent.com/2026/07/22/mjS8/20260723004146063.png)


# 参考
[^1]: WebRTC VAD — <https://webrtc.googlesource.com/src/+/refs/heads/main/common_audio/vad/>
[^2]: [Silero VAD: pre-trained enterprise-grade Voice Activity Detector.](https://github.com/snakers4/silero-vad)
[^3]: [FunASR: A Fundamental End-to-End Speech Recognition Toolkit.](https://arxiv.org/abs/2305.11013)"
[^4]: [Powerset multi-class cross entropy loss for neural speaker diarization.](https://github.com/pyannote/pyannote-audio)
[^5]: [FireRedASR2S: A State-of-the-Art Industrial-Grade All-in-One Automatic Speech Recognition System." arXiv:2603.10420, 2026.](https://arxiv.org/pdf/2603.10420)