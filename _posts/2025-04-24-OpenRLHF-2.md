...
- 强化学习
- OpenRLHF
description: 本文主要介绍OpenRLHF源码中PPO范式训练前的初始化配置，涉及DeepSpeed和vLLM配置。vLLM有Hybrid和标准资源调度模式，通过ray封装vLLM推理架构。还介绍了experience_maker.py中BaseExperienceMaker等定义，ppo_actor.py的trainer类，launcher.py中的分布式Actor相关内容，包括模型初始化等操作。
---
