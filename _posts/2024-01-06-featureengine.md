...
description: 数据和特征决定机器学习上限，特征选择与数据降维是降低数据维度的核心技术。特征选择旨在构建简单可理解模型、提升数据挖掘性能及准备干净数据，常用算法包括依赖学习算法预测性能的Wrapper方法（如SFS、SBS序列选择算法，易面临NP-hard问题）、基于变量排序的Filter方法（如ANOVA、互信息、相关系数、卡方检验、方差选择等）及融合两者的Embedded方法，另有Laplacian
  Score（通过图构建与拉普拉斯矩阵计算特征重要性）、Fisher Score（监督选取使同类特征相似、异类特征差异大的特征）等方法。数据降维则将高维数据投影到低维空间实现“压缩”（区别于特征筛选的“特征数量选择”），典型如PCA主成分分析，通过寻找低维线性子空间描述高维数据，有效简化数据结构。这些技术共同助力高维数据预处理，优化机器学习模型效率与可解释性。
---
