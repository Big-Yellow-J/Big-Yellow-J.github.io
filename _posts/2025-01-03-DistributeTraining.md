...
> **流水线并行**：因为模型分布在不同设备上（假设：$ld_1, ld_2, ld_3, ld_4, ld_5$），会有一个操作：将数据在拆分为不同`micro-batch`（这里假设为5，得到：$md_1,md_2,md_3,md_4,md_5 $），这样一来随着前向传播：$t_0$时：$ld_1$处理$md_1$；$t_2$时：$(ld_1, md_2), (ld_2, md_1)$。对比 **模型并行** ：假设$t_4$恰好`流水线并行`处理完数据，那么$t_0\rightarrow t_5$：$(ld_1, (md_1...md_5))$
