...
- cv-backbone
- 卷积网络
description: 'ConvNeXt是Meta提出的卷积神经网络模型，源自《A ConvNet for the 2020s》和《ConvNeXt V2: Co-designing
  and Scaling ConvNets with Masked Autoencoders》两篇论文，核心是通过借鉴ViT（Vision Transformer）的超大感受野及各类优化trick，证明卷积网络在性能上可媲美Transformer。ConvNeXt
  v1主要通过模型调参与结构改进提升效果，包括macro design（调整block堆叠数量、采用4x4卷积核、通道数与Swin Transformer对齐至96）、Inverted
  bottleneck（借鉴ViT中MLP结构，先降后升通道）、Large Kernel Sizes（7x7卷积核）及Micro Design（激活函数换GELU、减少激活与归一化层、拆分采样层等），逐步提升准确率。ConvNeXt
  v2进一步引入Masked Autoencoder（MAE，随机移除60% 32x32图像块，采用FCMAE全局还原策略）和Global Response Normalization（GRN，通过跨通道全局平方和归一化解决特征图死亡或饱和问题），优化特征表达。该模型通过对比实验验证，借鉴Transformer的设计理念后，卷积网络在图像任务上表现优异，为卷积与Transformer融合提供新思路。'
---
...
<img src="https://s2.loli.net/2025/06/21/WZfCUH6FmnKJxgj.webp" alt="image" width="859" height="624" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/RsDEntGOxLZ6av7.webp" alt="image" width="553" height="712" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/FkBqSp3rX5uvGC1.webp" alt="image" width="709" height="311" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/CYmcDh9gniwbRKH.webp" alt="image" width="577" height="192" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/W1qf72cmXed9BEh.webp" alt="image" width="569" height="606" loading="lazy" decoding="async" />

总的来说这篇论文主要还是集中在玄学调参：1、对于ResNet去修改他的堆叠数量（1,1,9,1）；2、换更加大的卷积核（4x4，4）；3、更加少的激活函数核归一化处理（都只在1x1卷积之后进行操作）。提供如下几种模型
<img src="https://s2.loli.net/2025/06/21/NumwMVRf1ECkdp9.webp" alt="image" width="574" height="147" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/v4SZOY9IdlRQsu6.webp" alt="image" width="648" height="521" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/21/O9Zc8QTtDxWAazk.webp" alt="image" width="577" height="192" loading="lazy" decoding="async" />
