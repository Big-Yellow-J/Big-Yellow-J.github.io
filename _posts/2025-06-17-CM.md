...
（LCM蒸馏）训练过程中主要使用了3个模型：1、`teacher_model`；2、`unet`；3、`student_model`
1、首先是构建跳步迭代过程，而后去计算（公式-1） $c_{skip}$和 $c_{out}$以及：$f_\theta(x,t)=c_{skip}(t)x+ c_{out}(t)F_\theta(x,t)$ 对于其中的$x$ 以及 $F_\theta$ （代码中）分别表示的是$t$时刻 模型预测的输出和 $t-1$时刻的噪声图像（加噪反推可以直接计算出来）这样一来就得到（ `unet`模型计算 ）：`model_pred`；
2、对于**ODE solver计算**就和公式-1计算过程相似，只不过需要区分有文本编码和没有文本编码两种，最后得到（这个过程通过教师模型 `teacher_model` 处理）`pred_x0 = cond_pred_x0 + w * (cond_pred_x0 - uncond_pred_x0)` 以及 `pred_noise = cond_pred_noise + w * (cond_pred_noise - uncond_pred_noise)` 而后再通过扩散解噪过程得到$t$时刻的（**ODE Solver结果**）： `x_prev`；
3、计算loss通过`student_model`处理 `x_prev`而后反推得到 `pred_x_0`再去计算$f_\theta(x,t)=c_{skip}(t)x+ c_{out}(t)F_\theta(x,t)$（`c_skip * x_prev + c_out * pred_x_0`）
