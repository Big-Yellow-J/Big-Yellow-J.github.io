...
## Claude Code安装使用
### 安装环境准备
目前大部分的skills等都会用到python因此就需要去安装python等，因此需要去安装python，除此之外还需要权重npm等，因此：**在win电脑上更加建议直接使用wsl去搭建Claude Code**，[WSL安装方式](https://www.runoob.com/linux/windows-wsl-linux.html)，安装完毕之后其他命令就和linux安装命令相同
### 安装与卸载
...
**如果是基于WSL**安装，那么以此执行如下命令：
```cmd
curl -fsSL https://claude.ai/install.sh | bash
echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc && source ~/.bashrc
# 输入 claude 看看能不能启动
vim ~/.claude/settings.json # 去修改配置 将你的DeepSeek 订阅写入
