...
<img src="https://s2.loli.net/2025/06/22/8s2hPy1D4orpvYi.webp" alt="image" width="886" height="311" loading="lazy" decoding="async" />
...
<img src="https://s2.loli.net/2025/06/22/EMaLQzBUm5sFtI1.webp" alt="image" width="531" height="586" loading="lazy" decoding="async" />
### SAM v2
SAM v2[^2]更像是SAM v1在视频邻域的泛化通过引入Memory结构（Memory Attention/bank来记录之前视频帧的特征），整个模型结构如下所示：
<img src="https://s2.loli.net/2025/06/21/GECuoyeilnI2KHO.webp" alt="image" width="1121" height="293" loading="lazy" decoding="async" />
**Memory Attention**（[代码](https://github.com/facebookresearch/sam2/blob/main/sam2/modeling/memory_attention.py#L102)）：将当前帧的特征与过去帧的特征和预测以及任何新的提示联系起来。通过堆叠了 L 个transformer模块（[Attention处理代码](https://github.com/facebookresearch/sam2/blob/2b90b9f5ceec907a1c18123530e92e794ad901a4/sam2/modeling/memory_attention.py#L93)），每个transformer块首先执行自注意力（self-attention），这允许模型关注当前帧内部的特征。
接着执行交叉注意力（cross-attention），这允许模型将当前帧的特征与存储在记忆库中的记忆（包括提示和未提示的帧）以及对象指针进行交互。这些记忆可以是之前帧的特征，也可以是之前预测的结果。
**memory encoder**（[代码](https://github.com/facebookresearch/sam2/blob/main/sam2/modeling/memory_encoder.py)）：通过使用卷积模块对输出掩码进行下采样，并将其与图像编码器的无条件帧嵌入相加，生成记忆，然后使用轻量级卷积层来融合信息。
**memory bank**：**最多N个最近帧的FIFO记忆队列来保留视频中目标对象的过去预测信息，并将提示信息存储在最多M个提示帧的FIFO队列中**。例如，在VOS任务中，初始掩码是唯一的提示，内存库始终保留第一帧的记忆以及最多N个最近(非提示)帧的记忆。两组记忆都以空间特征图的形式存储。
...
## Clip
Clip[^5]模型结构（论文里面提到的）也比较简单，其核心机制为：**核心机制是通过对比学习和嵌入空间对齐，将图像和文本映射到一个共享的语义空间中**
<img src="https://s2.loli.net/2025/06/21/vzix8HIcwAMDWdZ.webp" alt="image" width="1024" height="384" loading="lazy" decoding="async" />
