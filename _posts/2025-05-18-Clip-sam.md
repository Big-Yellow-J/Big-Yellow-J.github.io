...
- multimodal
show: true
description: 本文介绍多模态中常用的两种backbone——Clip和SAM。SAM有v1和v2版本，v1结构简单，涉及图像、提示、掩码解码等模块，能处理不确定提示输出多掩码；v2是v1在视频领域的泛化，有记忆注意力等结构。Clip通过对比学习和嵌入空间对齐，预训练时对齐文本与图像特征，使用时编码图像和文本计算结果。
---
