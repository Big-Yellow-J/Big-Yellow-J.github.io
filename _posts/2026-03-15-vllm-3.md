...
![](https://files.seeusercontent.com/2026/05/07/Lu9d/image20260322170632097.webp)
...
![20260323161425227](https://files.seeusercontent.com/2026/05/07/Pao5/image20260323161425227.webp)
...
**时间1**：缓存为空，输入request0（prompt为A-O）总共15个token。直接从block pool中分配了 4 个块（假设每个块只能缓存4个token）。其中 3 个已满那么就会被缓存（直接使用SHA256进行编码，编码逻辑是： `sha256( parent_hash + 本块tokens + extra )` 分别表示上一个列表、本block token、额外信息），第四个块部分填充了 3 个 token，那么此时被填满token的block直接建立hash id比如A-D对应ID=0，如果下次还有A-D的token输入可以直接从cache blocks直接复用（直接拿ID=0）
> 具体复用可以直接看上面显存分配过程，每层(K+V, num_blocks, block_size, num_kv_heads, head_size) 只需要去num_blocks去拿对应ID即可，除此之外虽然分了block但是计算时候还是复用的，比如说计算E时候会直接那 ABCD的cache内容

![](https://files.seeusercontent.com/2026/05/28/n6fC/20260528213242518.png)
**时间2**：开始decode处理，这样就会对 request0 使块3填满，并请求新块以继续解码，这个时候有4块被填满那么就会在cache block中建立4个hash
![](https://files.seeusercontent.com/2026/05/28/U5ut/20260528213333101.png)
...
```
![](https://files.seeusercontent.com/2026/05/28/o0sD/20260528213355348.png)
> 值得注意的是，只有当前面token都相同才会命中缓存，比如说Reques2输入是：BACDEFGHxxxx就是有EFGH相同也不会命中缓存，以为计算需要复用Request2开始的token式BACD完全和Request0的ABCD不同那么就不要复用EFGH的缓存

**时间 4**：请求 0 完成并释放。块 2、3 和 4 按反向顺序添加到空闲队列（但块 2 和 3 仍被缓存）。块 0 和 1 未添加到空闲队列，因为它们正在被请求 1 使用
![](https://files.seeusercontent.com/2026/05/28/zXr5/20260528213423308.png)
**时间 5**：请求 1 完成并释放。
![](https://files.seeusercontent.com/2026/05/28/Z9bo/20260528213443724.png)
...
![](https://files.seeusercontent.com/2026/05/28/Ivz9/20260528213459725.png)
