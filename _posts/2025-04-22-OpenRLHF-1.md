...
里面的 $r(\theta)=\pi_{\theta}(a|s)/\pi_{old}(a|s)$ 就是概率比值，上面代码中： `log_probs_from_logits(output["logits"][:, :-1, :], sequences[:, 1:], temperature=self.temperature)`计算的就是：$log(\pi_{\theta}(a|s))$，在具体代码中：
