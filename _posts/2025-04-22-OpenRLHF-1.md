...
- 强化学习
- OpenRLHF
description: 本文介绍OpenRLHF模型框架设计，涉及actor.py、model.py、loss.py。actor.py加载模型，处理生成及对数概率计算；model.py含CriticModel与RewardModel，前者评估动作价值，后者输出整句奖励；loss.py有PPO的策略与价值损失，三类模型协同保障强化学习流程。
---
