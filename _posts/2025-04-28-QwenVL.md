...
- rope
- attention
description: 本文介绍Qwen-vl系列模型，包括Qwen2-vl和Qwen2.5-vl。Qwen2-vl用动态分辨率、2x2拼接token及多模态旋转位置编码；Qwen2.5-vl在视觉编码器（改进ViT、2D-RoPE）和MLP（patch解析分组后入MLP）有改进，还涉及模型预训练（含多种数据及处理）、后训练（监督微调、直接偏好优化等）及代码对比等内容。
---
