...
- WGAN
- WGAN_GP
description: 本文主要介绍生成图像的基础模型，重点讲解GAN。GAN含生成器G与判别器D，通过博弈训练，G捕获数据分布，D判断样本来源。其数学原理涉及目标公式推导，训练分两部分。GAN有训练不稳定、模型坍塌等缺点，还提及用WGAN等改进，并用MNIST测试代码展示效果，最后总结GAN优化目标及相关改进。
---
