...
tags:
- 模型训练
description: 模型训练常通过flash-attn、混合精度优化训练速度，但普遍缺乏对CPU、GPU等设备性能瓶颈的系统分析。PyTorch训练数据流遵循磁盘→内存→CPU→GPU计算→内存→磁盘路径，常见瓶颈分为五类：CPU瓶颈表现为GPU利用率跳变、部分CPU核打满，多为Dataloader参数配置问题；GPU计算瓶颈为利用率高但吞吐量低，需参考MFU指标判断；I/O瓶颈表现为训练初始慢后续提速，磁盘读写负载高；多卡通信瓶颈表现为部分卡利用率低，NCCL的AllReduce操作占比超30%；另有Python解释器、GIL导致的框架开销。可通过bpytop、nvidia-smi、iotop做宏观性能监控，用torch.profiler定位耗时点，对应调整Dataloader参数、使用NVIDIA
  DALI、算子融合、梯度压缩等方案优化。
---
...
**那么有如下问题**：1、我的数据（dataset）中我的数据有1T，设备处不了如何做？2、如何取定义自己的采样器？
1、处理大批量数据：
https://apxml.com/zh/courses/advanced-pytorch/chapter-3-optimization-training-strategies/large-dataset-strategies；
https://apxml.com/zh/courses/how-to-build-a-large-language-model/chapter-8-building-managing-large-scale-datasets/distributed-file-systems
https://www.aidoczh.com/speechbrain/tutorials/advanced/data-loading-for-big-datasets-and-shared-filesystems.html
2、处理不同的采样器，最明显的就是在文本中文本的长度是不一致的，如果要使用batch_siez>1那么就势必需要用smapler策略用mask去提高效率
宏观指标最简单方式直接用time.time()即可，然后可以取看如何用一个函数封装time中这样就不用重复的两次time.time了
## 宏观指标分析
