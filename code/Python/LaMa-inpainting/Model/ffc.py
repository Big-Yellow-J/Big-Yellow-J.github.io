...
from Model.base import get_activation, BaseDiscriminator
from Model.spatial_transform import LearnableSpatialTransformWrapper
...
class FFCResNetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, 
                 ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
...
if __name__ == '__main__':
    model1 = FFCResNetGenerator(input_nc= 4, output_nc= 3)
    model1.to("cuda:1")

    model2 = FFCNLayerDiscriminator(input_nc= 3)
    model2.to("cuda:1")

    x = torch.randn(32, 4, 512, 512).to("cuda:1")
    y = model1(x)
    print(f"Model 1: {y.shape}")
    y2_1, y2_2 = model2(y)
    print(y2_1.shape)
    # from datasets import load_dataset

    # ds = load_dataset("saitsharipov/CelebA-HQ", cache_dir=)